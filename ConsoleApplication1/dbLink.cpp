...
static inline std::string getExamplePath()
{
    ...
    std::string tempStr= filePath.substr(0, filePath.length() - std::string("dblink.cpp").length());
    return tempStr;
}
...
//  std::cout << "SQlite3 version " << SQLITE_VERSION << std::endl;
    ...
    try
    {
        ...
        //FILE* fp = fopen_s(filename_logo_png.c_str(), "rb");
        FILE* fp = NULL;
        fopen_s(&fp, filename_logo_png.c_str(), "rb");
        if (NULL != fp)
        {
            char  buffer[16 * 1024];
